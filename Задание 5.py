...
def purchase():
    ...
    while True:
        ...
        if flag == False:
            ...
            while True:
                ...
                general_numb = int(dict.get(name[i])[2])
                if numb1.isdigit():
                    if int(numb1) == 0:
                        break
                        ...
                        numb.append(int(numb1))
                        print("Заказ успешно оформлен!")
                        dict[name[i]][2] = str(int(dict.get(name[i])[2]) - int(numb1))
                        i+=1
                        break
                else:
    ...
    print("Вы приобрели : \n")
    while i < len(name):
        price = price + int(dict.get(name[i])[1]) * numb[i]
        print(name[i] + "\nСостав:" + dict.get(name[i])[0] + "; цена(бел. руб) - " +
              dict.get(name[i])[1]
              + "; приобретённое количество: " + str(numb[i]) + " шт.")
        i+=1
